...
## Problem
Analyze server cluster VMs in PVE: VM 111 (Java web server, Ubuntu), VM 112 (SQL database server, openEuler), and an nginx LXC container. Reconstruct the "鲸易元MALL管理系统" website. Answer 14 questions.

## Evidence
- PVE VM 111 — Java/Spring Boot web application server (Ubuntu)
- PVE VM 112 (sqlserver) — MySQL database in Docker (openEuler)
- LXC container 110 — nginx reverse proxy
...
**Important**: Do NOT start VM 112 before extracting — running it modifies the disk and changes hash.
...
→ *(visible in forensic tool analysis)*
...
```
4. Add hosts entries for domains → PVE nginx IP
5. Replace admin Bcrypt hash in MySQL with known hash (e.g., Bcrypt of "123456")
6. Login to admin backend

## Key Takeaways
- **Disk hash BEFORE boot**: Always extract virtual disk images before starting VMs — running modifies the disk
...
- **Network interface mismatch**: Config says `ens36` but physical device is `enp6s18` — use `nmcli` to bind
...
- **MySQL port 13306**: Non-standard port — check Docker port mapping
