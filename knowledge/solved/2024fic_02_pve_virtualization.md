...
## Evidence
- sys.E01 (~8.6GB) — PVE system partition
- data.E01 (~34GB) — PVE data partition
...
Check network configuration in PVE web UI → Node → Network.
→ **192.168.100.0/24**

### Q5: "120(Luck)" VM smbios uuid
Double-click Luck VM → Options → SMBIOS settings.
→ **e9990cd6-6e60-476c-bd37-1a524422a9a8**

### Q6: User "Lu2k" VM permissions count
Server View → Permissions tab. Count Lu2k entries.
→ **4**
...
Check PVE node → Task History, filter for `qmdestroy`.
...
Task History → filter download tasks.
...
Check each VM for snapshot info. node2 has snapshot "安装手机".
→ **D. 2024-03-13 9:43:23**

## Key Takeaways
- **Image mount order matters**: sys.E01 first, data.E01 second for PVE emulation
- **PVE Task History**: Searchable/filterable — use task type filters (qmdestroy, download, etc.)
